...
public class ProgressiveLogger implements LogHandler
{
    ...
    public void log(LogLevel level, String text) {
        ...
        {
            case debug:
                renderedText = Pipe.apply(" [D] " + text)
                    .pipe(Colors::applyStyle, AttributedStyle.BOLD.foreground(170, 255, 50))
                    .pipe(ProgressiveLogger::addTimestamp)
                    .result();
                break;
            case err:
                renderedText = Pipe.apply(" [E] " + text)
                    .pipe(Colors::applyStyle, AttributedStyle.BOLD.foreground(255, 15, 15))
                    .pipe(ProgressiveLogger::addTimestamp)
                    .result();
                break;
            case info:
                renderedText = Pipe.apply(" [I] " + text)
                    .pipe(Colors::applyStyle, AttributedStyle.BOLD.foreground(0, 177, 210))
                    .pipe(ProgressiveLogger::addTimestamp)
                    .result();
                break;
            case warn:
                renderedText = Pipe.apply(" [W] " + text)
                    .pipe(Colors::applyStyle, AttributedStyle.BOLD.foreground(255, 255, 0))
                    .pipe(ProgressiveLogger::addTimestamp)
                    .result();
                break;
            default:
    ...
    {
        return Pipe.apply(new Date())
            .pipe(date -> dateFormat.format(date))
            .pipe(Colors::applyStyle, AttributedStyle.BOLD.foreground(128, 128, 128))
