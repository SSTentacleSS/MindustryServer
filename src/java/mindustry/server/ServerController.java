...
public class ServerController implements ApplicationListener {
	...
	protected static DateTimeFormatter dateTime = DateTimeFormatter.ofPattern(
		"MM-dd-yyyy HH:mm:ss"
	...
	protected void setup(String[] args) {
		...
		handler
			.getCommandList()
			.forEach(Main.IO.commandsRegistry::registerCommand);
		...
		Core.app.post(
			() -> {
				...
				if (!Config.startCommands.string().isEmpty()) {
					String[] startup = Strings
						.join(" ", Config.startCommands.string())
						.split(",");
		...
		try {
			maps.setShuffleMode(
				ShuffleMode.valueOf(Core.settings.getString("shufflemode"))
			);
		} catch (Exception e) {
		...
		Events.on(
			...
			event -> {
				...
				Map map = nextMapOverride != null
					? nextMapOverride
					: maps.getNextMap(lastMode, state.map);
					...
					if (autosaveCount.get(Config.autosaveSpacing.num() * 60)) {
						...
						String mapName =
							(
								state.map.file == null
									? "unknown"
									: state.map.file.nameWithoutExtension()
							).replace(" ", "_");
						String date = autosaveDate.format(LocalDateTime.now());

						Seq<Fi> autosaves = saveDirectory.findAll(
	...
	protected void registerCommands() {
		handler.register(
			"help",
			"[command]",
			"Display the command list, or get help for a specific command.",
			arg -> {
				if (arg.length > 0) {
					Command command = handler
						.getCommandList()
						.find(c -> c.text.equalsIgnoreCase(arg[0]));
					if (command == null) {
		...
		handler.register(
			...
			arg -> {
				...
				boolean def =
					arg.length > 0 &&
					(arg[0].equals("default") || arg[0].equals("all"));
				...
				if (!maps.all().isEmpty()) {
					...
					if (all.isEmpty()) {
						info(
							"No custom maps loaded. &fiTo display built-in maps, use the \"@\" argument.",
							"all"
						);
					} else {
						info("Maps:");
				...
				info(
					"Map directory: &fi@",
					customMapDirectory.file().getAbsoluteFile().toString()
				);
			}
		);
		...
		handler.register(
			...
			arg -> {
				...
				if (maps.all().size > beforeMaps) {
					info(
						"@ new map(s) found and reloaded.",
						maps.all().size - beforeMaps
					);
	...
	private void handleCommandString(String line) {
		...
		if (response.type == ResponseType.unknownCommand) {
			...
			if (closest != null && !closest.text.equals("yes")) {
				err(
					"Command not found. Did you mean \"" + closest.text + "\"?"
				);
						...
						try {
							...
							serverSocket.bind(
								new InetSocketAddress(
									Config.socketInputAddress.string(),
									Config.socketInputPort.num()
								)
							);
							while (true) {
								Socket client = serverSocket.accept();
								info(
									"&lkReceived command socket connection: &fi@",
								...
								info(
									"&lkLost command socket connection: &fi@",
									serverSocket.getLocalSocketAddress()
								);
								socketOutput = null;
							}
							...
							err(
								"Command input socket already in use. Is another instance of the server running?"
							);
						} catch (IOException e) {
							if (
