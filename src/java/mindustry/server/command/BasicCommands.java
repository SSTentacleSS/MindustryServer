	...
	public void register(CommandHandler handler) {
		...
		handler.register(
			...
			arg -> {
				net.closeServer();
				if (
					StateController.lastTask != null
				) StateController.lastTask.cancel();
		...
		handler.register(
			...
			arg -> {
				...
				logic.reset();
				StateController.lastMode = preset;
				Core.settings.put(
					"lastServerMode",
					StateController.lastMode.name()
				);
				try {
					world.loadMap(
						result,
						result.applyRules(StateController.lastMode)
					);
